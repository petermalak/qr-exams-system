@extends('admin.layouts.index')
@section('content')
    <script src=" {{ asset('./admin/html5-qrcode.min.js') }}"></script>

    <h1 style="text-align: center;"> Scanner (sahmamsa school)</h1>
    @if (Request::route()->getPrefix() == 'door-entrance')
        <h2 style="text-align: center;">Door Entrance</h2>
    @elseif(Request::route()->getPrefix() == 'door-exit')
        <h2 style="text-align: center;"> Door exit</h2>
    @elseif(Request::route()->getPrefix() == 'alhan')
        <h2 style="text-align: center;"> Alhan</h2>
    @elseif(Request::route()->getPrefix() == 'coptic')
        <h2 style="text-align: center;"> Coptic</h2>
    @elseif(Request::route()->getPrefix() == 'taks')
        <h2 style="text-align: center;"> Taks</h2>
    @elseif(Request::route()->getPrefix() == 'agbeya')
        <h2 style="text-align: center;"> Agbeya</h2>
    @endif
    <div class="row" style="text-align: center;">
        <div class="col">
            <div style="width: 70%; height:50%; margin: auto;" id="reader"></div>
        </div>
    </div>
    <div class="container">
        <form name="add-blog-post-form" id="add-blog-post-form">
            <div class="row">
                <div class="col-12">
                    <div class="form-group">
                        <input required type="text" id="student_id" name="student_id" class="form-control form-elements">
                    </div>
                </div>
            </div>
        </form>
        <div class="row">
            <div class="col-12 text-center">
                <button type="submit" class="btn btn-success" id="get_data_button">Submit</button>
            </div>
        </div>
    </div>

    <div class="card-body table-responsive p-0">
        <table class="table table-bordered table-striped table-responsive-stack" id="tableOne" hidden>
            <thead>
                <tr>
                    <th>Student ID</th>
                    <th>Name</th>
                    <th>Group Number</th>
                    <th>In Hall</th>
                    @if (Request::route()->getPrefix() == '/alhan' ||
                            Request::route()->getPrefix() == 'alhan' ||
                            Request::route()->getPrefix() == '/door-entrance' ||
                            Request::route()->getPrefix() == 'door-entrance' ||
                            Request::route()->getPrefix() == '/door-exit' ||
                            Request::route()->getPrefix() == 'door-exit')
                        <th>Alhan</th>
                    @endif
                    @if (Request::route()->getPrefix() == '/coptic' ||
                            Request::route()->getPrefix() == 'coptic' ||
                            Request::route()->getPrefix() == '/door-entrance' ||
                            Request::route()->getPrefix() == 'door-entrance' ||
                            Request::route()->getPrefix() == '/door-exit' ||
                            Request::route()->getPrefix() == 'door-exit')
                        <th>Coptic</th>
                    @endif
                    @if (Request::route()->getPrefix() == '/taks' ||
                            Request::route()->getPrefix() == 'taks' ||
                            Request::route()->getPrefix() == '/door-entrance' ||
                            Request::route()->getPrefix() == 'door-entrance' ||
                            Request::route()->getPrefix() == '/door-exit' ||
                            Request::route()->getPrefix() == 'door-exit')
                        <th>Taks</th>
                    @endif
                    @if (Request::route()->getPrefix() == '/agbeya' ||
                            Request::route()->getPrefix() == 'agbeya' ||
                            Request::route()->getPrefix() == '/door-entrance' ||
                            Request::route()->getPrefix() == 'door-entrance' ||
                            Request::route()->getPrefix() == '/door-exit' ||
                            Request::route()->getPrefix() == 'door-exit')
                        <th>Agbeya</th>
                    @endif
                    <th>Out Hall</th>
                </tr>
            </thead>
            <tbody>
                <td id="student_id_cell"></td>
                <td id="student_name"></td>
                <td id="student_group_number"></td>
                <td id="student_inhall"></td>
                @if (Request::route()->getPrefix() == '/alhan' ||
                        Request::route()->getPrefix() == 'alhan' ||
                        Request::route()->getPrefix() == '/door-entrance' ||
                        Request::route()->getPrefix() == 'door-entrance' ||
                        Request::route()->getPrefix() == '/door-exit' ||
                        Request::route()->getPrefix() == 'door-exit')
                    <td id="student_alhan"> </td>
                @else
                    <td hidden id="student_alhan"> </td>
                @endif
                @if (Request::route()->getPrefix() == '/coptic' ||
                        Request::route()->getPrefix() == 'coptic' ||
                        Request::route()->getPrefix() == '/door-entrance' ||
                        Request::route()->getPrefix() == 'door-entrance' ||
                        Request::route()->getPrefix() == '/door-exit' ||
                        Request::route()->getPrefix() == 'door-exit')
                    <td id="student_coptic"></td>
                @else
                    <td hidden id="student_coptic"> </td>
                @endif
                @if (Request::route()->getPrefix() == '/taks' ||
                        Request::route()->getPrefix() == 'taks' ||
                        Request::route()->getPrefix() == '/door-entrance' ||
                        Request::route()->getPrefix() == 'door-entrance' ||
                        Request::route()->getPrefix() == '/door-exit' ||
                        Request::route()->getPrefix() == 'door-exit')
                    <td id="student_taks"></td>
                @else
                    <td hidden id="student_taks"> </td>
                @endif
                @if (Request::route()->getPrefix() == '/agbeya' ||
                        Request::route()->getPrefix() == 'agbeya' ||
                        Request::route()->getPrefix() == '/door-entrance' ||
                        Request::route()->getPrefix() == 'door-entrance' ||
                        Request::route()->getPrefix() == '/door-exit' ||
                        Request::route()->getPrefix() == 'door-exit')
                    <td id="student_agbeya"></td>
                @else
                    <td hidden id="student_agbeya"> </td>
                @endif
                <td id="student_outhall"></td>
            </tbody>
        </table>
    </div>
    <script type="text/javascript">
        // Flag to prevent multiple requests
        var isRequestInProgress = false;

        function onScanError(errorMessage) {}

        var html5QrcodeScanner = new Html5QrcodeScanner("reader", {
            fps: 10,
        });
        html5QrcodeScanner.render(onScanSuccess, onScanError);

        $("#get_data_button").click(function() {
            sendRequest();
        });

        function onScanSuccess(qrCodeMessage) {
            if (!isRequestInProgress) {
                // Prevent multiple requests
                isRequestInProgress = true;
                sendRequest(qrCodeMessage);
            }
        }

        function sendRequest(qrCodeMessage = null) {
            $.ajax({
                data: {
                    "_token": "{{ csrf_token() }}",
                    'student_id': qrCodeMessage || document.getElementById('student_id').value,
                    'prefix': '{{ Request::route()->getPrefix() }}'
                },
                type: 'POST',
                url: "{!! route('exam-attendances.show') !!}",
                statusCode: {
                    400: function(response) {
                        let student = response.responseJSON.student;
                        let student_attendance = response.responseJSON.student_attendance;
                        add_error_message();
                        add_values_to_table(student, student_attendance);
                        isRequestInProgress = false;
                    },
                    200: function(response) {
                        console.log(response);

                        if (response) {
                            let student = response.student;
                            let student_attendance = response.student_attendance;
                            add_success_message();
                            add_values_to_table(student, student_attendance);
                        }
                        isRequestInProgress = false;
                    },
                    401: function(response) {
                        alert(
                            'Error: This Student not recorded from Entrance Door\nNote: You Have to Make a Scan from the entrance');
                        isRequestInProgress = false;
                    },
                    403: function(response) {
                        alert(
                            'Attendance already recorded for this student');
                        isRequestInProgress = false;
                    }
                },
                error: function() {
                    isRequestInProgress = false; // Reset the flag in case of error
                }
            });
        }

        function convert_bool_to_string(value) {
            return value == 1 ? 'Examed' : 'Not Examed';
        }

        function add_cell_style(element_id) {
            let element = document.getElementById(element_id);
            if (!element) {
                console.error("Element not found with ID:", element_id);
                return;
            }
            let html = element.innerHTML.trim(); // Get the innerHTML and trim any whitespace
            $("#" + element_id).removeClass(); // Remove any existing classes
            // Check if the content contains "Examed" (case-sensitive)
            if (html.includes('Examed')) {
                element.classList.add('green_background');
            } else if (html.includes('Not Examed')) {
                element.classList.add('red_background');
            } else {
                element.classList.add('yellow_background');
            }
        }

        function add_values_to_table(student, student_attendance) {
            $('#student_id').html(student.id);
            $('#student_id_cell').html(student.id);
            $('#student_name').html(student.name);
            $('#student_group_number').html(student.group_number);
            $('#student_inhall').html(convert_bool_to_string(student_attendance.in_hall));
            add_cell_style('student_inhall');
            $('#student_alhan').html(student_attendance.alhan);
            add_cell_style('student_alhan');
            $('#student_coptic').html(student_attendance.coptic);
            add_cell_style('student_coptic');
            $('#student_taks').html(student_attendance.taks);
            add_cell_style('student_taks');
            $('#student_agbeya').html(student_attendance.agbeya);
            add_cell_style('student_agbeya');
            $('#student_outhall').html(convert_bool_to_string(student_attendance.out_hall));
            add_cell_style('student_outhall');
            $('table').removeAttr('hidden');
        }

        function add_success_message() {
            let message = $(
                '<div class="alert alert-success m-1" id="success-message" style="margin:15px; height:2.5rem" role="alert"> <p class="justify-content-center mb-3">Successfully</p> <br></div>'
                );
            $(".content-wrapper").prepend(message);
            $('#success-message').fadeOut(2000, function() {
                $(this).remove();
            });
        }

        function add_error_message() {
            let message = $(
                '<div class="alert alert-danger m-1" id="error-message" style="margin:15px; height:2.5rem" role="alert"> <p class="justify-content-center mb-3">There is Exam missing</p> <br></div>'
                );
            $(".content-wrapper").prepend(message);
            $('#error-message').fadeOut(2000, function() {
                $(this).remove();
            });
        }

        $('#student_id').keypress(function(e) {
            var key = e.which;
            if (key == 13) { // the enter key code
                $('#get_data_button').click();
                return false;
            }
        });
    </script>

    <style>
        body {
            background-color: #cccccc;
            table-layout: fixed;
        }

        .button {
            padding: 15px 25px;
            font-size: 24px;
            text-align: center;
            cursor: pointer;
            outline: none;
            color: #fff;
            background-color: #527c6d;
            border: none;
            border-radius: 15px;
            box-shadow: 0 9px #999;
            width: 25%;
            display: block;
            margin-left: auto;
            margin-right: auto;
        }

        .button:hover {
            background-color: #3e8e41
        }

        .button:active {
            background-color: #3e8e41;
            box-shadow: 0 5px #666;
            transform: translateY(4px);
        }

        .form-elements {
            width: 30%;
            text-align: center;
            height: 50px;
            padding: 12px 20px;
            box-sizing: border-box;
            border: 2px solid rgb(156, 89, 89);
            border-radius: 4px;
            background-color: #f8f8f8;
            font-size: 16px;
            resize: none;
            display: block;
            margin-left: auto;
            margin-right: auto;
        }

        .green_background {
            background-color: #27a800
        }

        .red_background {
            background-color: #FF0000
        }
    </style>
@endsection
