...
class AlhanExamAttendancesDataTable extends DataTable
{
    /**
    ...
    public function query(ExamAttendance $model): Builder
    {
        ...
        $examStudents = BookingExams::all()->filter(function ($exam) use ($today) {
            ...
            return $examDate && $examDate->format('Y-m-d') === $today && $exam->type == 'alhan';
        ...
        return $model->newQuery()
            ...
            })->where('in_hall', 1)->where('alhan',0); // Ensure they are in the hall
    }
    ...
    {
        return 'AlhanExamAttendances_' . date('YmdHis');
    }
}
